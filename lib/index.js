...
function _prepareOptions (options) {
  ...
  if (!storage || !utils.isFunction(storage.load) || !utils.isFunction(storage.store)) {
    ...
    tempOptions.storage = require('./memory-storage');
  }
  ...
  return function handleRequest(req, res, next) {
    ...
    res.end = function (data, encoding) {
      ...
      if (headerSession.session) {
        ...
        if (options.debug) {
          console.log('request "%s" needs no header session', req.url);
        }
         _end.call(res, data, encoding);
      }
    };
...
function _handleMetrics(options, req, res) {
  ...
  if (!utils.isFunction(options.storage.info)) {
    res.sendStatus(400);
    return;
  }
  options.storage.info().then(
    function (sessionInfoList) {
      res.send(sessionInfoList);
    }
  );
}
...
module.exports = function (app, options) {
  ...
  var
    ...
    rootPath = thisOptions.root || '/restful';

  app.use(rootPath, _initialize(thisOptions));
